...
- **Tavolo di gioco** (`apps/player-activity`): Discord Activity React. Viewer tattico stateless; sync HTTP polling (2 s) via API Gateway.
- **packages/shared**: contratti TypeScript + Zod (World State, sync messages, Discord IDs).
- **infrastructure**: AWS CDK (web hosting, HTTP API, DynamoDB handshake/sync, S3/CloudFront canon, Bedrock stub). Vedi [ADR 0013](docs/adr/0013-aws-web-hosting-and-dns.md), [ADR 0014](docs/adr/0014-session-http-polling-sync.md).
...
5. **UUID v7** per tutti gli ID persistenti. Sync tattico HTTP con payload `SyncEnvelope<T>` / `MqttMessage` (nome storico) in `packages/shared`.
...
| Contratti sync HTTP                  | `packages/shared/src/sync/`                                                                       |
...
| Web hosting / DNS CDK                | [ADR 0013](docs/adr/0013-aws-web-hosting-and-dns.md), `infrastructure/README.md`                  |
