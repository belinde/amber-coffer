...
use crate::commands::vault_validate::validate_session_status;
...
#[tauri::command]
pub async fn get_session(id: String, state: State<'_, AppState>) -> Result<Option<Session>, AppError> {
...
pub async fn create_session(
    ...
    let number = match input.number {
        Some(n) if n > 0 => {
            ensure_session_number_available(state.pool(), &input.campaign_id, n, None).await?;
            n
        }
        _ => next_session_number(state.pool(), &input.campaign_id).await?,
    };
...
pub async fn update_session(
    ...
    let existing = get_session(input.id.clone(), state.clone())
        .await?
    ...
    get_session(input.id, state)
        .await?
        .ok_or_else(|| AppError::Internal("session update succeeded but row missing".into()))
}

#[tauri::command]
pub async fn delete_session(id: String, state: State<'_, AppState>) -> Result<(), AppError> {
