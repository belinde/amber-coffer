...
#[tauri::command]
pub async fn get_lore_note(id: String, state: State<'_, AppState>) -> Result<Option<LoreNote>, AppError> {
...
pub async fn update_lore_note(
    ...
    let existing = get_lore_note(input.id.clone(), state.clone())
        .await?
    ...
    get_lore_note(input.id, state)
        .await?
        .ok_or_else(|| AppError::Internal("lore note update succeeded but row missing".into()))
}

#[tauri::command]
pub async fn delete_lore_note(id: String, state: State<'_, AppState>) -> Result<(), AppError> {
