...
use crate::db::validate::{
    ensure_campaign_exists, validate_image_links,
};
...
fn images_root(app: &AppHandle) -> Result<PathBuf, AppError> {
    let data_dir = app
        .path()
        .app_data_dir()
...
fn image_dir(app: &AppHandle, campaign_id: &str, image_id: &str) -> Result<PathBuf, AppError> {
    Ok(images_root(app)?.join(campaign_id).join(image_id))
}
...
pub async fn create_campaign_image(
    ...
    validate_image_links(state.pool(), &input.campaign_id, &input.links).await?;
...
pub async fn update_campaign_image(
    ...
    let existing = get_campaign_image(input.id.clone(), state.clone())
        .await?
        .ok_or_else(|| AppError::NotFound("campaign_image".into()))?;

    validate_image_links(state.pool(), &existing.campaign_id, &input.links).await?;
    ...
    get_campaign_image(input.id, state)
        .await?
        .ok_or_else(|| AppError::Internal("campaign image update succeeded but row missing".into()))
}

#[tauri::command]
pub async fn delete_campaign_image(id: String, app: AppHandle, state: State<'_, AppState>) -> Result<(), AppError> {
    let existing = get_campaign_image(id.clone(), state.clone())
        .await?
        .ok_or_else(|| AppError::NotFound("campaign_image".into()))?;
    ...
    let dir = image_dir(&app, &existing.campaign_id, &existing.id)?;
    if dir.exists() {
...
pub async fn attach_campaign_image_file(
    ...
    let existing = get_campaign_image(image_id.clone(), state.clone())
        .await?
        .ok_or_else(|| AppError::NotFound("campaign_image".into()))?;
    ...
    let dest_dir = image_dir(&app, &existing.campaign_id, &existing.id)?;
    ...
    let local = format!("{}/{}/{}", existing.campaign_id, existing.id, file_name);
    ...
    get_campaign_image(image_id, state)
        .await?
...
pub async fn resolve_campaign_image_path(
    ...
    if !local_path.starts_with(&campaign_prefix) {
        return Err(crate::validation_issue::generic_invalid(&["local"]));
    }

    let root = images_root(&app)?;
